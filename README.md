...
- `MapHomeScreenTest`（5 项）：首页三个入口、底部加号、无地图引导、玻璃弹窗、新建地图端到端
- `SettingsScreenTest`（2 项）：设置页三档外观与语言 / 关于入口
- `ThemeModeTest`（2 项）：外观模式解析（跟随系统 / 强制浅色 / 强制深色）

共 15 项，全部通过。

> 弹窗改成页面内浮层（液态玻璃）之后，弹窗交互已经可以自动化测试了。
> 全屏看图仍是独立窗口，那部分只能在真机验证。
...
- **更新安装后桌面图标可能不刷新**：小米 / HyperOS 桌面会缓存每个包的图标记录，覆盖安装后可能仍显示旧图标或系统兜底图标。
  图标资源本身没问题（可用系统「应用信息」页对照：那页显示的才是包内真实图标），重启手机或强制停止桌面即可刷新。
  排查时不要反复改图标格式——那是在猜，先重启验证。
...
- **所有弹窗统一改成液态玻璃**：背后内容实时模糊 + 半透明玻璃卡片 + 高光边框 + 弹簧放大入场，
  点遮罩关闭（新建地图 / 重命名 / 删除确认 / 编辑字段 / 添加道具）
- **所有卡片和按钮按下时轻微发光**：三层渐隐光晕（用描边实现，任何 Android 版本都能画），
  入口卡片按各自主题色发光，删除按钮发红光
- 弹窗从独立窗口改为页面内浮层，弹窗交互终于能自动化测试
- 测试增至 21 项（新增 `ThemeModeTest`、`SettingsScreenTest`、玻璃弹窗与新建地图端到端用例）
